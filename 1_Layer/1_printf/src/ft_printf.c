...
/*   Updated: 2023/06/01 16:19:20 by regea-go         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	format(char const *fmt, va_list args, int i)
{
	...
	if (fmt[i] == 'c')
		...
		size += ptrhx(va_arg(args, unsigned long int));
	else if (fmt[i] == 'd')
		...
		size += to_hex(va_arg(args, unsigned int));
	else if (fmt[i] == 'X')
		size += to_hex_up(va_arg(args, unsigned int));
	else if (fmt[i] == '%')
		size += ft_putchar_fd(fmt[i], 1);
	return (size);
}
